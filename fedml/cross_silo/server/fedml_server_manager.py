...
class FedMLServerManager(FedMLCommManager):
    ...
    def handle_messag_connection_ready(self, msg_params):
        ...
        self.data_silo_index_list = self.aggregator.data_silo_selection(
            self.args.round_idx, self.client_total,
    ...
    def handle_message_client_status_update(self, msg_params):
        ...
        #logging.info(client_status)
        #logging.info(self.client_online_mapping)
        #logging.info(str(msg_params.get_sender_id()))
        if client_status == "ONLINE":
            self.client_online_mapping[str(msg_params.get_sender_id())] = True #compare this to list of selected clients and only on match add it to online list and message ready once done

            logging.info("self.client_online_mapping = {}".format(self.client_online_mapping))
            #logging.info(self.client_id_list_in_this_round)
        #logging.info(list(self.client_online_mapping.keys()))
